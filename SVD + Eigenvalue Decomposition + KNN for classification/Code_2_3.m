...
TS_1 = audioread("Data/Music_Part_3\TS1.mp3");
TS_2 = audioread("Data/Music_Part_3\TS2.mp3");
TS_3 = audioread("Data/Music_Part_3\TS3.mp3");
KL_1 = audioread("Data/Music_Part_3\KL1.mp3");
KL_2 = audioread("Data/Music_Part_3\KL2.mp3");
KL_3 = audioread("Data/Music_Part_3\KL3.mp3");
CP_1 = audioread("Data/Music_Part_3\CP1.mp3");
CP_2 = audioread("Data/Music_Part_3\CP2.mp3");
CP_3 = audioread("Data/Music_Part_3\CP3.mp3");
...
%removing 5 seconds from start and end of each song as it is mostly silence
...
for i = 1:9
    for j = 5:34
        train_snippets((i-1)*30 + j-4,:) = songs(i,(j-1)*178657 + 1:j*178657 - 1);
    end
end
for j = 35:37
    for k = 1:3
        for i = 1:9
                test_snippets(i + (k-1)*9,:) = songs(i,(j-1)*178657 + 1:j*178657 - 1);
        end
    end
end
...
for i = 1:270
    ...
    Sgt_spec_train_matrix(i,:) = reshape(Sgt_spec_train, 1, length(tslide)*n);
end
...
for i = 1:27
    ...
    Sgt_spec_test_matrix(i,:) = reshape(Sgt_spec_test, 1, length(tslide)*n);
end
%% SVD
...
for k = 1:3
    ...
    for i = 1:9
        ...
        for j = 1:3
            dist = sum(abs(avg_projections(:,j) - test_projection(:,i + (k-1)*9)));
            if dist < min_dist
                min_dist = dist;
                min_index = j;
            end
...
% csvwrite("Spectogram_train_3.csv",Sgt_spec_train_matrix);
% csvwrite("Spectogram_test_3.csv",Sgt_spec_test_matrix);